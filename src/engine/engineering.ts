 ...
 * (E4.2.6), and any left unspent when the segment ends are lost (E4.2.10) —
 * `forfeitUnspentArming` records them as the segment closes, so that the
 * points a mid-round battery buys (B2.5.6) are spendable while the ones a
 * captain declined to spend stay dead.
 */
export function armingPointsAvailable(ship: ShipState, weaponId: string): number {
  ...
  return Math.max(0, generated - spent - (ship.armingForfeited[weaponId] ?? 0))
}

/**
 * E4.2.10, enforced: whatever a weapon system has not spent when the Resource
 * Allocation Segment closes is written off. Without this, opening the arming
 * controls for the battery rules would quietly resurrect every leftover.
 */
export function forfeitUnspentArming(ship: ShipState): void {
  for (const line of ship.form.functions) {
    if (line.kind !== 'weapon' || !line.weaponSystemId) continue
    ship.armingForfeited[line.weaponSystemId] = 0
    const unspent = armingPointsAvailable(ship, line.weaponSystemId)
    if (unspent > 0) ship.armingForfeited[line.weaponSystemId] = unspent
  }
}
